...
metaJson = JSON.parse(fs.readFileSync(blogPath + '/meta.json', 'utf8'));
listJson = JSON.parse(fs.readFileSync(blogPath + '/list.json', 'utf8'));

for (var i = 0; i < listJson.list.length; i++) {
	accumulate = postTemplate.replace('{{AUTHOR}}', metaJson['blogName']).replace('{{URL}}', 'http://' + metaJson['domain'] + metaJson['path'] + '/?p=' + i).replace('{{TITLE}}', listJson.list[i]['Title']).replace('{{DATE}}', listJson.list[i]['Time']).replace('{{POST}}', fs.readFileSync(blogPath + '/db/' + i + '.txt', 'utf8')) + accumulate;
};
