...
var loadPost = function (pid) {
	...
	ajaxContent[thisId].onload = function () {
		...
		if (postIndex.list[pid].T == '') {
			document.getElementById('post' + pid + 'h2').remove();
		} else {
			document.getElementById('post' + pid + 'title').innerHTML = postIndex.list[pid].T;
		...
		document.getElementById('post' + pid + 'link').innerHTML = postIndex.list[pid].D;
		document.getElementById('post' + pid + 'link').href = './?p=' + pid;
		if (postId != null && postIndex.list[pid].T != '') {
			document.title = postIndex.list[pid].T.replace(/<[\/a-z]*?>/g, '') + ' — ' + blogName;
		} else if (postId != null && postIndex.list[pid].T == '') {
